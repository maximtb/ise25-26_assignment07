...
public class ReviewController extends CrudController<Review, ReviewDto, Long> {
    ...
    @Operation
    protected @NonNull DtoMapper<Review, ReviewDto> mapper(){return reviewDtoMapper;}
    ...
    public @NonNull ResponseEntity<ReviewDto> getById(
            @Parameter(description = "Unique identifier of the review to retriev.")
                @PathVariable Long id) {return super.getById(id);}
    ...
    public @NonNull ResponseEntity<ReviewDto> create(
            @Parameter(description = "Data of the review to create.", required = true)
                @RequestBody @Valid ReviewDto reviewDto){return super.create(reviewDto);}

    @Operation
    @CrudOperation(operation = UPDATE, resource = REVIEW)
    @PutMapping("/{id}")
    public @NonNull ResponseEntity<ReviewDto> update(
            @Parameter(description = "Unique identifier pf the review to update.", required = true)
                @PathVariable Long id, @Parameter(description = "Data of the review to update.", required = true)
                @RequestBody @Valid ReviewDto reviewDto){return super.update(id,reviewDto);}
    ...
    public @NonNull ResponseEntity<List<ReviewDto>>filter(
            @Parameter(description = "ID of the POS whose reviews to retrieve.",required = true)
                @RequestParam("pos_id")Long posId,
            @Parameter(description = "Whether to only return approved reviews.", required = true)
                @RequestParam("approved") Boolean approved){
        var reviews = reviewService.filter(posId,approved).stream().map(reviewDtoMapper::fromDomain).toList();
